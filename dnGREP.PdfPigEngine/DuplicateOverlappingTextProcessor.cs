namespace dnGREP.Engines.PdfPig
{
    ...
    /// <para>Modified from PdfPig's implementation which removes valid doubled letters.</para>
    /// </summary>
    public static class DuplicateOverlappingTextProcessor
    {
        ...
        public static IReadOnlyList<Letter> Get(IEnumerable<Letter>? letters)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                var duplicates = cleanLetters.Where(l =>
                    !l.Value.Equals(" ", StringComparison.Ordinal) &&
                    l.Value.Equals(letter.Value, StringComparison.Ordinal) &&
                    l.FontName.Equals(letter.FontName, StringComparison.Ordinal)); // do other checks?

                if (duplicates.Any())
                {
                    double width = letter.EndBaseLine.X - letter.StartBaseLine.X;
                    double tolerance = width / (letter.Value.Length == 0 ? 1 : letter.Value.Length) / 3.0;
